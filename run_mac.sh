...
if [ -f "$SETTINGS_FILE" ]; then
    # Try to extract language from settings file if grep is available
...
echo -e "${CYAN}KASETA Audio Tool - macOS Environment Check${NC}"
echo "================================================"
...
if ! command -v python3 &> /dev/null; then
    ...
    echo "For macOS, install Python using Homebrew:"
...
if [ "$PYTHON_VERSION_MAJOR" -lt 3 ] || ([ "$PYTHON_VERSION_MAJOR" -eq 3 ] && [ "$PYTHON_VERSION_MINOR" -lt 6 ]); then
    ...
    echo "Update Python with: brew upgrade python"
    exit 1
fi
...
if ! command -v brew &> /dev/null; then
    echo -e "${YELLOW}Homebrew not found. Recommended for managing dependencies.${NC}"
    echo "Install Homebrew with command:"
    echo '/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"'
    echo "Continuing without Homebrew, but dependency issues may occur."
fi
...
if ! command -v ffmpeg &> /dev/null; then
    ...
    if command -v brew &> /dev/null; then
        ...
        brew install ffmpeg
        if [ $? -ne 0 ]; then
            echo -e "${RED}Error installing FFmpeg!${NC}"
            echo "Install manually with: brew install ffmpeg"
            exit 1
        fi
    else
        echo -e "${RED}Install FFmpeg manually:${NC}"
        echo "1. Install Homebrew: /bin/bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\""
...
if [ ! -d "venv" ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "Try installing virtualenv: pip3 install virtualenv"
        exit 1
    fi
fi
...
if ! pip list | grep -q "PyQt5"; then
    ...
    # macOS may have issues with PyQt5 installation via pip
    if command -v brew &> /dev/null; then
        echo -e "${YELLOW}Installing PyQt5 via Homebrew for reliability...${NC}"
        brew install pyqt5
    fi
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "For PyQt5 issues, try installing via Homebrew: brew install pyqt5"
        echo "Then install remaining dependencies: pip install pydub googletrans==3.1.0a0"
        exit 1
    fi
fi
